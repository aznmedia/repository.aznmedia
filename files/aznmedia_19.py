...
def get_m3u(url):
	...
	for thumb, name, url in match:
		if 'script.navi-x' in url:
			...
			navix = xbmc.translatePath('special://home/addons/script.navi-x')
			if os.path.exists(navix) == True: 
				addDir(name, url, mode, thumb, True)
			else:
				addDir(name, 'NaviX', 101, thumb, True)
		else:
...
def main():
	...
	if mysettings.getSetting('enable_adult_section') == 'true':
		addDir('[COLOR red][B]Adult XXX (18+)[/B][/COLOR]', 'adult', 98, ico('18'), True)
...
def install_repos(headTitle):
	...
	try:
		if KodiVersion > 17:
			...
			repoGroup = os.path.join(packagesLoc, RepoZip) #xbmc.translatePath('special://home/addons/packages/allinoneadult18.zip')
			#ReposDataLoc = os.path.expanduser(r'~\Desktop\allinoneadultuserdata18.zip')
			ReposDataLoc = mainloc + 'allinoneadultuserdata18.zip' #https://bitbucket.org/aznmedia/repository.azn.media/raw/master/RepoZips/allinoneadultuserdata18.zip
			RepoDataZip = os.path.basename(ReposDataLoc) #allinoneadultuserdata18.zip
			repoDataGroup = os.path.join(packagesLoc, RepoDataZip) #xbmc.translatePath('special://home/addons/packages/allinoneadultuserdata18.zip')
...
world = baseurl + 'worldtv.m3u'
otheradu = baseurl + 'adult/adultmainmenu.xml'
adultaddons = baseurl + 'adult/adultaddons.txt'
...
if mode == None or url == None or len(url) < 1:
	easyadultinstaller = os.path.join(datafiles, 'EasyAdultInstaller.txt')
	if os.path.exists(easyadultinstaller) == True:
		...
		if d:
			try:
				...
				try:
					...
					xbmcaddon.Addon('plugin.video.videodevil').setSetting(id='hide_warning', value='true')
					xbmcaddon.Addon('plugin.video.videodevil').setSetting(id='video_type', value='3')
				except:
					pass
				time.sleep(1)
				...
				update_repo()
				time.sleep(1)
				dialog.ok('Easy Installer [COLOR red]including Adult 18+[/COLOR]',"Cần phải reboot Kodi để hoàn thành việc setup. Chúc bạn thưởng thức vui vẻ!")
				xbmc.executebuiltin("XBMC.ActivateWindow(ShutdownMenu)")
			except:
				dialog.ok('Easy Installer [COLOR red]including Adult 18+[/COLOR]','[COLOR magenta][B]Có lỗi xảy ra. Vui lòng thử lại sau.[/B][/COLOR]')
		else:
			shutil.rmtree(datafiles)
			sys.exit(0)
	else:
